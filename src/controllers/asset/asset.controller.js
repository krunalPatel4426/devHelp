...
const addAsset = asyncHandler(async (req, res) => {
  const { assetName, img, description, assetLink, tags, isFree, focus } =
    req.body;
...
const getAssetByTag = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const assets = await Asset.find(tagQuery).select("-assetLink -averageRating -totalRatings -rating -reviews -__v");
